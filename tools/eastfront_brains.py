...
class DualBrain:
    ...
    def launch(self,kind,s,payload,cfg,key):
        ...
        if not cfg.get('key') or not cfg.get('model'):self.latest[kind]={'error':'provider_not_configured'};return
        ...
        self.pending[kind]={'future':self.pool.submit(self.invoke,kind,payload,cfg,call),'run':s['run_id'],'key':key,'call':call,'started':time.monotonic()};self.used[kind]+=1
    def tick(self,s,instance,acks):
        ...
        enabled=f.get('backend')=='dual_brain' and not s.get('paused') and not s.get('winner')
        for kind,task in list(self.pending.items()):
            ...
            try:
                ...
                if kind=='slow':
                    ...
                    command={'action':'eastfront_directive','sector':f['active_sector'],'intent':intent,'construction':policy,'provider':'typesafe_jev_fast'}
        ...
        if enabled:
            ...
            if now>=self.next_fast:
                ...
                cfg=self.lm.profiles.get('typesafe_jev',{})
        ...
        self.lm.frontier_status={'mode':'dual_brain','used':dict(self.used),'in_flight':list(self.pending),'latest':copy.deepcopy(self.latest),'disabled':sorted(self.disabled),'budgets':{'slow':64,'fast':300},'fast_interval_seconds':2}
