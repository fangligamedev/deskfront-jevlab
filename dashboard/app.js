...
const modes={game_ai:'游戏 AI',lm:'DeepSeek LLM',player:'玩家接管',agent:'外部 Agent'},states={idle:'待机',move:'移动',cover:'掩护',aim:'瞄准',fire:'射击',reload:'换弹',suppressed:'受压制',dead:'失去战斗力',melee:'刺刀近战'};
...
function render(){if(!state.units)return;
...
function renderLM(){
 ...
 $('#lmStatus').textContent=lm.configured?`DeepSeek LLM · ${statusText} · ${lm.model} · 本局调用 ${lm.used_requests||0}/${lm.max_requests} · 并发 ${lm.in_flight||0}`:'LLM 未配置：服务端需要 Key 与模型 ID';
 ...
 $('#lmDecisions').innerHTML=rows.map(u=>`<div class="lm-decision"><div><b>${escapeHTML(u.unit_id)}</b><span>${lmPhases[u.phase]||escapeHTML(u.phase)}</span></div><p>${u.origin==='local_fallback'?'本地自保（非模型指令）':'DeepSeek LLM'} · ${intents[u.intent]||'观察'} · ${actions[u.action]||escapeHTML(u.action||'等待结果')}${u.latency_ms?' · '+u.latency_ms+' ms':''}</p><small>${escapeHTML(u.reason||'正在读取该单位的真实战场状态')}${u.receipt&&!u.receipt.accepted?' · '+escapeHTML(messageText(u.receipt.message)):''}</small></div>`).join('')||'<p class="caption">该阵营尚无 LLM 决策。每个单位单独请求，回执以引擎为准。</p>';
}
...
const errorMessages={session_replaced:'另一游戏页面已接管服务，请在当前使用的页面操作；刷新此页可重新连接',stale_run:'对局已更换，请等待新战场连接后重试',expired_or_new_run:'旧对局指令已失效，请重新操作',command_timeout:'引擎未在 10 秒内回应，请检查游戏是否正常运行',engine_offline:'游戏引擎尚未连接，请等待画面加载',lm_not_configured:'请在本地服务配置 ARK_API_KEY 和模型 ID',match_finished:'本局已经结束，请先重新开始',action_busy:'士兵正在完成投掷或掩体动作，请稍后重试',infantry_required:'此指令只能用于步兵',no_living_units:'当前阵营没有存活单位',authority_denied:'当前控制方式没有此单位的操作权限',survival_override:'单位正在紧急自保，暂缓危险指令',invalid_target:'目标已阵亡或失效，等待模型重新选目标',stale_observation:'战况已变化，旧决策不再执行',stale_control_epoch:'控制权已变化，旧决策不再执行',action_unavailable:'动作当前不可用，等待重新决策',anti_armor_required:'需要反装甲武器，轻武器不能攻击坦克'};
function messageText(code){return errorMessages[code]||code}
async function startSession(){
 try{
  const native=new URLSearchParams(location.search).has('native');
  const response=await fetch(native?'/api/state':'/api/session',native?{}:{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
  ...
  if(!native)$('#game').src='/play/index.html?instance_id='+encodeURIComponent(instanceId);
  poll();
...
$('#startLLM').onclick=async()=>{
 ...
 try{
  ...
  toast('DeepSeek LLM 对战已启动：每名士兵和坦克独立决策');
