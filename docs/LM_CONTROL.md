# DeepSeek LM 独立队员控制（0.6.0）

## 当前能力

顶层和每个阵营均可选择 `game_ai`（游戏 AI）、`lm`（火山方舟 DeepSeek）、`agent`（外部 Agent）、`player`（玩家）。LM 对每个存活单位单独请求、单独维护最近两条意图；新增坦克也作为独立单位加入。没有用一条整队模型回复伪装为九个智能体。
...
6. 游戏本地自保可中断危险推进。重伤/强压制期间拒绝 LM 继续进攻或要求起身，撤离会清除追击目标，防止攻击追击覆盖逃跑路径。坦克保留低血与近距离反坦克威胁的紧急脱离。

玩家点选/框选或切换模式会改变控制权版本；LM → 玩家 → LM 也不能复用旧回复。局次变化、死亡、暂停、观察超过 300 tick 或离线均使返回失效。切回游戏 AI 后恢复协调器。

## 可调用动作与状态

引擎动作全集在 [AGENT_ACTION_STATES.md](AGENT_ACTION_STATES.md)，机器目录为 `GET /api/action-catalog`。LM 可选择相同 13 个战术命令；另有 `wait`，仅保留现有意图，不伪造引擎命令或回执。

步兵可以在单条 LM 移动等命令中附 `posture`，表达“趴下爬过去”；校验失败不会部分改变姿态。坦克不接受姿态或手雷。所有单兵指令保留队友自己的目标、路线与意图。
...
默认每局最多 120 次请求、单次超时 6 秒、输出最多 256 token。模型失败采用退避；预算用完后不会继续扣调用额度，UI 仍显示预算和本地保命。重新开始一局会重新计数。应用重启不跨进程记账，因此这个上限是每局护栏，并非账户级消费上限。
...
该试验启动独立临时控制服务和真实 Godot 无头引擎，按相同地图/初始游戏种子做三方全游戏 AI 与三方全 LM 的短时比较。LM 网络延迟、输出和实时随机消费并非确定性逐帧重放；这不是 LM 对游戏 AI 的胜率比赛，不能据此宣称孰优孰劣。
...
模型错误或预算用尽时，正在安全推炮/进楼的单位继续当前任务；其他单位使用本地保命。局部危险仍能打断任务。`error_codes` 将 JSON/动作失效与供应商错误分开统计。当前仍没有模型间通信、共享长程指挥和隐藏敌情；此版本不把独立 LM 宣称为成熟小队指挥系统。
