    ...
    def tick(self,s,instance,acks):
        ...
        if enabled:
            ...
            if planning and now>=getattr(self,'next_slow',0):
                ...
                context={'frontier':planning,'recent':[recent[i] for i in sorted(recent)][-3:],'previous_validation_error':self.latest.get('slow',{}).get('error',''),'wave':f.get('wave',1),'squad':[{'hp':u['hp'],'weapon':u['weapon']} for u in s['units'] if u['faction']=='green' and u['hp']>0]}
                prompt='你是玩具兵无尽东线的慢脑战役规划师。输出未来三段可执行方案，不只是模板名字。每段选模板、2至3名守军weapon只能为rifle/smg/rocket（禁止cannon等未实现武器），分配不同的整数掩体station(0/1/2)、defense(entrench/crossfire/fallback)、construction(balanced/north_first/south_first/dig_first/sandbag_first)、双方坦克是否出场/开战后延迟0至20游戏秒/战术role(support/push)。armor的green和red无论enabled是true或false都必须完整提供enabled/delay/role三个键，禁用时填delay:0,role:support。第一段必须安排双方坦克。任意一段只要green.enabled=true，该段红方defenders必须至少一名rocket反制。previous_validation_error若非空，纠正该错误后重新给出完整三段。红军应据壕防守，不要在开阔地待机。三段模板不得重复，第一段不可重复recent末段。Godot拥有塑料骨骼士兵、现有坦克、可破坏沙包与浅壕工位；没有真实地下地形或任意生成的资产。严格返回JSON：{"strategy":"简短总体战术","sectors":[{"template":"候选ID","defenders":[{"weapon":"rifle","station":0},{"weapon":"rocket","station":2}],"defense":"entrench","construction":"dig_first","armor":{"green":{"enabled":true,"delay":2,"role":"support"},"red":{"enabled":true,"delay":4,"role":"support"}},"reason":"该段战术目的"},另外两段同结构]}。'
