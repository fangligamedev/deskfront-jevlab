...
const base=process.env.DESKFRONT_URL||'http://127.0.0.1:8768',out=path.resolve(__dirname,'../output/asset-application');
(async()=>{const browser=await chromium.launch({headless:true,args:['--use-angle=metal','--enable-gpu']}),page=await browser.newPage({viewport:{width:1720,height:1150}}),checks=[],errors=[];
...
await command({action:'pause',value:true});await command({action:'reinforce'});await page.locator('#phase').filter({hasText:'坦克抵达'}).waitFor();await until(d=>d.state.units.some(u=>u.faction==='green'&&u.available_actions.includes('man_at_gun')));await page.locator('.faction[data-team="green"] .faction-line').click();await page.locator('#manGun').click();await until(d=>d.state.at_guns.find(g=>g.faction==='green').crew_id!=='');
