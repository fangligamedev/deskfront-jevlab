...
func _initialize() -> void:
	...
	for filename in ["toy-soldier","office-worker"]:
		...
		for clip in player.get_animation_list():
			...
			for track in range(a.get_track_count()):
				...
				if path.get_subname_count()>0:
					...
					if bn.to_lower()=="root" and a.track_get_type(track)==Animation.TYPE_POSITION_3D:
		...
		if filename=="toy-soldier":ResourceSaver.save(player.get_animation_library(""),"res://output/infantry-library.tres")
		model.queue_free()
