...
async function startSession(forceNew=false){
 try{
  ...
  if(!attach)$('#game').src='/play/index.html?instance_id='+encodeURIComponent(instanceId)+((window.DeskfrontViews?.mode==='eastfront'||new URLSearchParams(location.search).get('campaign')==='eastfront')?'&campaign=eastfront':'');
...
function renderFlagWarning(){
 ...
 alarm.hidden=Boolean(state.eastfront?.enabled)||!flag?.owner;
