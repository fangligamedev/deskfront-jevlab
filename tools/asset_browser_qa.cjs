...
const out=path.resolve(__dirname,'../output/integration');
(async()=>{
 ...
 try{
  const prev=(await state()).run_id;await page.goto(base);let s=await until(s=>s.run_id!==prev&&s.version==='0.4.0');
  ...
  for(const faction of ['green','blue','red'])await command({action:'control',faction,mode:'player'});
  ...
  await command({action:'move',faction:'green',unit_ids:['green-1'],position:[.29,1.40]});
  ...
  await page.getByRole('button',{name:'◉ 投掷手雷',exact:true}).click();s=await until(s=>(s.combat_fx.launched.grenade||0)>0);
  check(s.units.filter(u=>u.faction==='green').some(u=>u.grenades===1),'Real grenade UI consumes inventory after release');
