...
(()=>{
 ...
 function renderRow(row){
  ...
  if(row.request?.questions)section('JEV / 结构化问题',row.request.questions);
 ...
 async function refresh(){
  ...
  try{
   ...
   el('liveLogStatus').dataset.error='false';el('liveLogStatus').textContent=!live?'引擎离线 · 保留最后日志':!lm.configured?'LLM 未配置 · 当前只能本地自保':items.length?'真实输入 → 模型指令 → 引擎回执':'等待本局 LLM 请求；旧局日志在「完整日志」中';
 ...
 window.renderDebugConsole=()=>{
  ...
  for(const row of debug.units||[]){
   ...
   const cmd=row.command||{},source=cmd.source==='local_fallback'?'本地自保':cmd.source==='lm'||cmd.source==='deepseek_lm'?'LLM':cmd.source||modes[row.control];
   ...
   c.append(node('small',`最近已执行指令依据：${cmd.reason||'暂无已接受指令'}${row.control==='lm'&&cmd.source==='console'?'（现已交还 LLM）':''}`));
